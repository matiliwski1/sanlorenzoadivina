...
function cargarPartido(partido) {
  ...
  plantillaActual.forEach(slot => {
    ...
    // Mostrar solo código de posición en el círculo
...
function abrirPosicion(pos) {
  ...
  const jugador = partidoActual.jugadores.find(j => j.pos === pos);
  const guiones = jugador ? Array.from(normalizar(jugador.apellido)).map(c => c === ' ' ? ' ' : '_').join(' ') : '';
  panelTituloEl.textContent = slot.label;
  panelEl.classList.remove('oculta');
 
  // Mostrar guiones debajo del título
  let guionesEl = panelEl.querySelector('.panel-guiones');
  if (!guionesEl) {
    guionesEl = document.createElement('p');
    guionesEl.className = 'panel-guiones';
    panelTituloEl.after(guionesEl);
  }
  guionesEl.textContent = guiones;
  renderIntentos();
  inputApellido.value = '';
  inputApellido.focus();
}
